    ...
    steps: [
      {
        ...
        icon: Radio,
        what: 'Agents talk over a single task lifecycle — submitted, working, input-required, completed or failed — streaming status updates and returning typed artifacts. That is what lets the UI show progress per agent instead of one long spinner.',
    ...
    steps: [
      {
        ...
        icon: Database,
        what: 'One state object, checkpointed by LangGraph, backs the whole run: conversation history, memory, retrieved documents and every artifact produced so far. Handing a task from one agent to the next never loses the thread — and a crashed run resumes instead of restarting.',
    ...
    steps: [
      {
        ...
        icon: Merge,
        what: 'The supervisor collects the artifacts, reconciles them, and decides whether the original request is actually satisfied. If a task failed or came back asking for input, it retries, reroutes, or asks you — it does not paper over the gap.',
